class Car{
					...
					this.goods.forEach((goodsValue)=>{
						if(resValue.goodsId == goodsValue.id){
							str += `<tr data-id="${resValue.goodsId}">
										...
										<td>${resValue.name}</td>
										<td>${resValue.price}</td>
										<td><input min="1" type="number" value="${goodsValue.num}"></td>
										<td class="mymoney">${resValue.price * goodsValue.num}</td>
										<td><span>删除</span></td>
									</tr>`
						}
					})
		...
		new Car({
			url:"http://localhost:9001/data/goods.json",
			tbody:$("tbody"),
			foor:$(".foor"),
			jia:$("#jia")
		})
